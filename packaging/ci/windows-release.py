...
installer = OUTPUT / f'CloudStream-PC-{VERSION}-Windows-x64-Setup.exe'
run([sys.executable, ROOT / 'tooling/packaging/build-windows-installer.py', runtime, installer, '--version', VERSION], log='installer-package.txt')
...
assert json.loads((installed / 'cloudstream-build.json').read_text()) == {'version': VERSION, 'package': 'windows-setup'}
assert json.loads((extracted / 'cloudstream-build.json').read_text()) == {'version': VERSION, 'package': 'windows-zip'}
import winreg
...
(EVIDENCE / 'installer-lifecycle.json').write_text(json.dumps({'install_version': VERSION, 'exe_matches_portable': True,
