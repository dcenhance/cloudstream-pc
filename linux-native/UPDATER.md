...
Keep release asset names consistent with `updates/ReleasePolicy.h`; all five existing Preview 4 names are covered. Upload new assets normally so GitHub computes a SHA-256 digest. Do not replace package-format semantics beneath an existing filename. Older builds without the updater need one manual installation of a release containing this feature. No release, tag, commit, upload, or installed application was changed while developing this feature.
